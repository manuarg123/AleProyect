...
router.get('/', async(req, res) => {
    ...
    Paciente.find((err, listar_personas) => {
        if (err) {
            ...
            res.render('templates/home', {status: req.session.login, pacientes: listar_personas})
        }
    })
})
...
router.post('/paciente', async(req, res) =>{
    ...
    Paciente.find((err, listar_personas) => {
        if (err) {
            ...
            req.session.login = true
            res.redirect('/api/usuario')
            res.render('templates/home', {status: true, pacientes: listar_personas})
        }
    })
}); 
...
router.get('/pacientes/editar/:id', async (req, res) => {
    if (req.session.login) {
        if (req.params.id.match(/^[0-9a-fA-F]{24}$/)) {
            ...
            res.render('templates/editar_paciente', {status: req.session.login,paciente})
        }
...
router.get('/historias_clinicas/:id', (req, res) => {
    if (req.session.login) {
        res.render('templates/historias_clinicas', {status: req.session.login, id: req.params.id});
    } else {
        res.render('templates/login', {status: false})
    }   
})
...
router.get('/form_historias_clinicas/:paciente_id', (req, res) =>{
    if (req.session.login) {
        res.render('templates/form_historias_clinicas', {status: req.session.login, id: req.params.paciente_id});
    } else {
        res.render('templates/login', {status: false})
    }
})
